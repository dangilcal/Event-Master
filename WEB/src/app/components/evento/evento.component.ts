...
export class EventoComponent implements OnInit {
  ...
  pdf() {
    //let data = document.getElementById('pdfDescargar');

    // html2canvas(data as any).then(canvas => {
    //     var imgWidth = 15;
    //     const contentDataURL = canvas.toDataURL('image/png');
    //   let pdfData = new jsPDF('l', 'mm', 'a4');
    //     var imgProps = (pdfData as any).getImageProperties(contentDataURL);
    //     var imgWidth = pdfData.internal.pageSize.getWidth() - 2 * imgWidth;
    ...
    if (this.evento?.nombre) {
      ...
      pdfData.text("Fecha del inicio del evento  --> " + this.eventoFechaInicio, 20, 60);
      pdfData.text("Fecha del final del evento  --> "+this.eventoFechaFinal, 20, 70);
      var logo = new Image();
